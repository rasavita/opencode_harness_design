...
const { projectEncodingBlock, encContexts } = require('../.opencode/scripts/scaffold-encoding');
...
test('scaffold wiring: template carries the placeholder and writeClaudeMd fills it', () => {
  assert.match(read('.opencode/templates/agents-md.template.md'), /\{project-encoding\}/);
  assert.match(read('.opencode/scripts/scaffold-apply.js'), /projectEncodingBlock\(render\.buildManifest\(profile\)\)/);
});
