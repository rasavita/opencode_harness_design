...
const ON = { HARNESS_AUTO_CONTINUE: '1' };
...
// settings.json now ships HARNESS_AUTO_CONTINUE=1 and would leak into the runner.
const OFF = { HARNESS_AUTO_CONTINUE: '' };
...
function statePath(projectDir, name) {
  return path.join(projectDir, '.opencode', 'state', name);
}
...
test('no-ops entirely when HARNESS_AUTO_CONTINUE is off, even with work remaining', async () => {
