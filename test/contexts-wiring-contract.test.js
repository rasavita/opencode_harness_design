...
test('the contexts check is opt-in (off unless architecture.contexts is set)', () => {
  const { loadContextConfig, checkContextContent } = require(path.join(ROOT, '.opencode/hooks/lib/contexts.js'));
...
test('manifest marks bounded-context-rules active', () => {
  ...
  assert.strictEqual(s.wired_at, '.opencode/hooks/lib/contexts.js');
