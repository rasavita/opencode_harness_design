...
} = require('../.opencode/scripts/checkpoint-state');
...
test('resume stops cleanly when the task budget is exhausted', () => {
  const root = fixture();
  const file = path.join(root, '.opencode', 'state', 'task-envelope.json');
  ...
  fs.writeFileSync(path.join(root, '.opencode', 'state', 'budget-start'), '1000');
