...
test('pre-commit wires the mutation gate, scoped to /auto builds', () => {
  ...
  const hook = read('.opencode/git-hooks/pre-commit');
  assert.match(hook, /gate-registry/, 'pre-commit must dispatch through gate-registry');
  const registry = read('.opencode/hooks/lib/gate-registry.js');
  ...
  const quality = read('.opencode/hooks/lib/gates-verification.js');
...
test('mutation-gate CLI reuses the lib, which drives the existing smoke runner', () => {
  assert.ok(fs.existsSync(path.join(ROOT, '.opencode/scripts/mutation-gate.js')));
  assert.ok(fs.existsSync(path.join(ROOT, '.opencode/hooks/lib/mutation-gate.js')));
  const cli = read('.opencode/scripts/mutation-gate.js');
  assert.match(cli, /require\('\.\.\/hooks\/lib\/mutation-gate'\)/, 'CLI must use the tested lib');
  const lib = read('.opencode/hooks/lib/mutation-gate.js');
...
test('the lib owns the orchestrator and is require-safe', () => {
  const lib = require(path.join(ROOT, '.opencode/hooks/lib/mutation-gate.js'));
  assert.strictEqual(typeof lib.runMutationOnFiles, 'function');
  // Importing the CLI must not run main() (no process.exit on require).
  assert.doesNotThrow(() => require(path.join(ROOT, '.opencode/scripts/mutation-gate.js')));
});

test('package.json exposes the mutation script and manifest marks it active', () => {
  const pkg = JSON.parse(read('package.json'));
  assert.strictEqual(pkg.scripts.mutation, 'node .opencode/scripts/mutation-gate.js --staged');
