...
function hasFreshContextPackReceipt(projectDir, maxAgeMs) {
  const receiptPath = path.join(projectDir, '.opencode', 'state', RECEIPT_NAME);
...
function contextSearchWarning(projectDir, ti, cfg) {
  ...
  return {
    ...
    message:
      ...
      `  node .opencode/scripts/context-pack.js --diff --budget 1600 "<your question>"\n` +
...
function unconstrainedSearchWarning(projectDir, command) {
  ...
  return {
    ...
    message:
      ...
      `  Prefer: node .opencode/scripts/context-pack.js --diff --budget 1600 "<question>"\n` +
