...
function hookErrorAdvisory(stateDir) {
  ...
  return `hook-errors.log has ${lines.length} new entr${lines.length === 1 ? 'y' : 'ies'} — a quality gate crashed and failed open (last: "${lines[lines.length - 1]}"). Review .opencode/state/hook-errors.log`;
}
...
function learningAdvisories(stateDir) {
  ...
  if (fs.existsSync(learnedRulesPath)) {
    ...
    if (ruleCount >= 10) {
      out.push(`learned-rules.md has ${ruleCount} rules — consider reviewing and promoting stable patterns to AGENTS.md`);
    }
  }
  ...
  if (fs.existsSync(failuresPath)) {
    ...
    if (failLines.length >= 5) {
      out.push(`failures.md has ${failLines.length} entries — recurring patterns should become AGENTS.md rules or hook enforcement`);
    }
  }
  for (const [name, limitMB] of [['iteration-log.md', 1], ['telemetry-ledger.jsonl', 5]]) {
    ...
    if (fs.existsSync(p)) {
      ...
      if (sizeMB > limitMB) {
        out.push(`${name} is ${sizeMB.toFixed(1)}MB — archive older entries via node .opencode/scripts/archive-state.js`);
      }
    }
  }
  return out;
  ...
  if (advisories.length > 0) {
    process.stdout.write(
      ['Session learnings review:', ...advisories.map((s) => `  - ${s}`),
        'Apply learnings: run /claude-md-management:revise-claude-md if that plugin is installed, otherwise edit AGENTS.md directly to promote these patterns.'].join('\n') + '\n'
    );
  }
} catch (err) {
