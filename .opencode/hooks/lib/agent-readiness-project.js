...
function modularityFreshnessPillar(root) {
  ...
  const marker = readJsonSafe(path.join(root, '.opencode', 'state', 'modularity-review-marker.json'));
...
function securityGovernancePillar(root, opts) {
  ...
  const scanPath = path.join(root, '.opencode', 'scripts', 'security-scan.js');
  if (!fs.existsSync(scanPath)) {
    return pillar('security-governance', 'Security & Governance', 'planned',
      ...
      'Run `/scaffold` (or copy .opencode/scripts/security-scan.js) to install the security-scan tooling.');
  }
