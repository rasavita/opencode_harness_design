...
const LEAD_TURN_CAVEAT = '> Lead-turn efficiency counts orchestrator turns, not tokens — exact '
  ...
  + 'cost, run `node .opencode/scripts/cost-report.js`.';
...
function buildScorecard(root) {
  const signals = {
    telemetry: summarizeTelemetry(readLines(root, '.opencode/state/telemetry-ledger.jsonl')),
    failures: parseFailures(readText(root, '.opencode/state/failures.md')),
    learnedRules: countRules(readText(root, '.opencode/state/learned-rules.md')),
    processRules: countRules(readText(root, '.opencode/state/process-rules.md')),
    flakeEvents: readFlakeCount(root),
    biting: analyzeBiting(root),
    baselines: {
      cycle: readBaselineNum(root, '.opencode/state/cycle-baseline.txt'),
      coupling: readBaselineNum(root, '.opencode/state/coupling-baseline.txt'),
      coverage: readBaselineNum(root, '.opencode/state/coverage-baseline.txt'),
      coverageJs: readBaselineNum(root, '.opencode/state/coverage-baseline-js.txt'),
    },
  };
