...
function architectureFitnessPillar(root) {
  const cyclePath = path.join(root, '.opencode', 'state', 'cycle-baseline.txt');
  const couplingPath = path.join(root, '.opencode', 'state', 'coupling-baseline.txt');
...
function testingSignals(root) {
  const coverageOk = fs.existsSync(path.join(root, '.opencode', 'state', 'coverage-baseline.txt'));
  const mutationOk = fs.existsSync(path.join(root, '.opencode', 'scripts', 'mutation-gate.js')) && hasNpmScript(root, 'mutation');
  const regressionOk = fs.existsSync(path.join(root, '.opencode', 'scripts', 'regression-gate.js')) &&
    fs.existsSync(path.join(root, '.opencode', 'scripts', 'local-regression-gate.js')) &&
