...
const TRUST_REL = path.join('.opencode', 'trust', 'issuers.json');
const AUTHORITY_REL = path.join('.opencode', 'authority');
...
function consumeCapability(root, capability, action, now = new Date()) {
  const dir = path.join(root, '.opencode', 'state', 'used-capabilities');
