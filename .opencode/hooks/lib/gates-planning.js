...
function validateContractShape(projectDir, group) {
  const schemaPath = path.join(projectDir, '.opencode', 'skills', 'evaluate', 'references', 'contract-schema.json');
  ...
  try {
    ...
    failBlock({
      ...
      fix: `re-negotiate the contract (node .opencode/scripts/validate-contract.js sprint-contracts/${group}.json to check).`,
      minTier: 'minimal',
    });
  }
  ...
  if (errors.length > 0) {
    failBlock({
      ...
      fix: `correct the contract (node .opencode/scripts/validate-contract.js sprint-contracts/${group}.json).`,
      minTier: 'minimal',
    });
  }
}
...
function checkVerificationMatrix(projectDir, group) {
  ...
  try {
    ...
    noteSkip('verification-matrix', 'gate script missing or unloadable from .opencode/scripts');
    return;
  }
  let verdict;
  try {
  ...
  if (!verdict.pass) {
    ...
    failBlock({
      ...
      detail: `${lines.join('\n')}\n${more}`,
      fix: 'run /evaluate to (re)generate runtime evidence and update the matrix, then retry the commit. Check: node .opencode/scripts/verification-matrix-gate.js --phase executed --group "' + group + '"',
      minTier: 'minimal',
    });
  }
}
