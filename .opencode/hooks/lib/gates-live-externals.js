...
function checkLiveExternalsGate(ctx) {
  ...
  try {
    ...
    noteSkip('live-externals', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
  ...
  if (!verdict.pass) {
    failBlock({
      ...
      fix: 'route the call through the boundary-test-doubles kit (Python or .ts sibling): replay the HTTP wrapper (ReplayTransport), use FakeLLMClient for model calls, and the DB transactional fixture — bind them under HARNESS_TEST_REPLAY=1. See .opencode/templates/boundary-doubles/. For a deliberately-live line (e.g. a staging smoke test), annotate it with `harness:live-ok`.',
