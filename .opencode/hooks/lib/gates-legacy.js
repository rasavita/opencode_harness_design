...
function resolveLegacyGate(ctx) {
  ...
  try {
    ...
    noteSkip('legacy-discipline', 'sensor script missing or unloadable from .opencode/scripts');
    return null;
  }
...
function checkSproutDiffGate(ctx) {
  ...
  try {
    ...
    noteSkip('sprout-diff', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
  const graphPath = path.join(projectDir, 'specs', 'brownfield', 'code-graph.json');
  ...
  if (!verdict.pass) {
    ...
    failBlock({
      ...
      fix:
        ...
        'in place (.opencode/skills/sprouting-instead-of-editing/SKILL.md).',
...
function checkAtFirstGate(ctx) {
  ...
  try {
    ...
    noteSkip('at-first', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
  const mapPath = path.join(projectDir, 'specs', 'design', 'component-map.md');
  ...
  if (verdict.storiesChecked.length > 0 && !verdict.pass) {
    ...
    failBlock({
      ...
      fix:
        ...
        'then node .opencode/scripts/record-at-red.js --story <id> --at-file <path> --test-cmd "<cmd>" to confirm it fails ' +
