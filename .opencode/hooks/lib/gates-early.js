...
function checkAmendmentProvenance(ctx) {
  ...
  try {
    ...
    noteSkip('amendment-provenance', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
...
function checkTestDeletionGate(ctx) {
  ...
  try {
    ...
    noteSkip('test-deletion-guard', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
...
function checkStubSmellGate(ctx) {
  ...
  try {
    ...
    noteSkip('stub-smell-gate', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
  const exec = gitExec(projectDir);
  const verdict = gate.checkStaged(exec);
...
function checkRefactorPurity(ctx) {
  ...
  failBlock({
    ...
    fix: 'a refactor commit changes no behavior — tests and snapshots must stay byte-identical. Split the behavioral work into its own commit (see .opencode/skills/keeping-refactors-pure/SKILL.md).',
    minTier: 'minimal',
  });
}
...
function checkOwnership(ctx) {
  ...
  try {
    ...
    noteSkip('ownership', 'sensor script missing or unloadable from .opencode/scripts');
    return;
  }
