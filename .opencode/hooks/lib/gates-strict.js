...
function checkCycleDetection(ctx) {
  ...
  const baselinePath = path.join(projectDir, '.opencode', 'state', 'cycle-baseline.txt');
  let graph;
  try {
...
function checkCouplingRatchet(ctx) {
  ...
  const { projectDir } = ctx;
  const graphPath = path.join(projectDir, 'specs', 'brownfield', 'code-graph.json');
  const baselinePath = path.join(projectDir, '.opencode', 'state', 'coupling-baseline.txt');
  let graph;
  try {
    graph = JSON.parse(fs.readFileSync(graphPath, 'utf8'));
  } catch (_) {
...
function checkSecurityBaseline(ctx) {
  ...
  const baselinePath = path.join(projectDir, '.opencode', 'state', 'security-baseline.txt');
