...
function checkScope(projectDir, filePath) {
  ...
  block(`BLOCKED: Write outside project directory: ${resolved}\nFix: Move the file to a location within the project directory or use .opencode/ for scaffold files.\n`);
}
...
function checkTaskEnvelope(projectDir, filePath) {
  ...
  if (loaded.state === 'invalid') {
    block(
      ...
      'Fix: run `node .opencode/scripts/task-envelope.js verify`, then recreate the envelope from the approved risk envelope.\n'
    );
  }
