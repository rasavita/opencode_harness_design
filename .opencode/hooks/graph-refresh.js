...
// Stop/SubagentStop — drain .opencode/state/graph-dirty.jsonl and patch
...
function renderDerivedDocs(projectDir, graph, rels) {
  const buildGraph = path.join(
    projectDir, '.opencode', 'skills', 'code-map', 'scripts', 'build_graph.js'
  );
...
function renderWiki(projectDir, graph) {
  const wiki = spawnSync('node', [
    path.join(projectDir, '.opencode', 'skills', 'code-map', 'scripts', 'code_wiki.js'),
...
function renderExplorer(projectDir, graph) {
  const viewer = path.join(projectDir, '.opencode', 'skills', 'code-map', 'scripts', 'graph_viewer.js');
...
function renderWikiBrowser(projectDir) {
  const viewer = path.join(projectDir, '.opencode', 'skills', 'code-map', 'scripts', 'wiki_viewer.js');
  if (!fs.existsSync(viewer)) return;
  const r = spawnSync('node', [
...
function runNavScripts(projectDir) {
  const navScripts = [
    path.join(projectDir, '.opencode', 'scripts', 'nav-index.js'),
    path.join(projectDir, '.opencode', 'scripts', 'nav-graph-index.js'),
    path.join(projectDir, '.opencode', 'scripts', 'nav-cochange.js'),
    path.join(projectDir, '.opencode', 'scripts', 'nav-concepts.js'),
    path.join(projectDir, '.opencode', 'scripts', 'nav-brownfield-maps.js'),
    path.join(projectDir, '.opencode', 'scripts', 'human-codebase.js'),
  ];
...
function refresh(projectDir, rels) {
  const indexer = path.join(
    projectDir, '.opencode', 'skills', 'code-map', 'scripts', 'code_index', 'code_index.py'
  );
