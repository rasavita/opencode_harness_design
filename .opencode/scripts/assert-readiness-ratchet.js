...
//
// Usage:
//   node .opencode/scripts/assert-readiness-ratchet.js \
...
//   baseline = <root>/.opencode/state/agent-readiness-baseline.json
//   root     = cwd
...
function main(argv = process.argv.slice(2)) {
  ...
  const baselinePath = path.resolve(
    arg(argv, '--baseline', path.join(root, '.opencode', 'state', 'agent-readiness-baseline.json'))
  );
