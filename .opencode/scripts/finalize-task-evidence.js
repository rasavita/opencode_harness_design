...
function finalize(root, now = new Date()) {
  ...
  const gateFile = path.join(root, '.opencode', 'state', 'gate-receipt.json');
  ...
  const receipt = stampEnvelope({
    ...
    run_id: readMarker(path.join(root, '.opencode', 'state'), 'current-run-id') || 'unassigned',
  ...
  const out = path.join(root, '.opencode', 'state', 'task-completion-receipt.json');
