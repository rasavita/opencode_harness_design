...
// Refresh .opencode/state/agent-readiness-baseline.json from a fresh report.
...
//   node .opencode/scripts/update-readiness-baseline.js [--root path] [--force]
...
function main(argv = process.argv.slice(2)) {
  ...
  const baselinePath = path.join(root, '.opencode', 'state', 'agent-readiness-baseline.json');

  const gen = spawnSync(process.execPath, [path.join(root, '.opencode', 'scripts', 'agent-readiness.js')], {
    cwd: root,
    encoding: 'utf8',
  });
  if (gen.status !== 0) {
