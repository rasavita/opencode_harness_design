...
function issue(root, command, argv) {
  ...
  const out = value(argv, '--out', path.join(root, '.opencode', 'authority', kind, `${signed.receipt_id}.json`));
...
function main() {
  ...
  const root = value(argv, '--root', process.env.OPENCODE_PROJECT_DIR || process.cwd());
  try {
