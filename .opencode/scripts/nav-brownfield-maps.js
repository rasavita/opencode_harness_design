...
function layerGuess(p) {
  ...
  if (/\/(hooks|scripts|skills)\//.test(s) || p.startsWith('.opencode/')) return 'tooling';
  return 'other';
}
...
function buildMaps({ projectDir = process.cwd(), goal = null } = {}) {
  ...
  const codebase = [
    ...
    '- Context pack: `node .opencode/scripts/nav-query.js pack --budget 1600 "<goal>"`',
    '',
  ].join('\n');
  ...
  const strategy = [
    ...
    '## First safe steps',
    '',
    '1. `node .opencode/scripts/nav-query.js pack --diff --budget 1600 "<goal>"`',
    '2. Read only `read_next` ranges; clarify if `confidence` is low.',
    '3. Coverage preflight on target symbols.',
    '4. Impact-scoped regression: `node .opencode/scripts/local-regression-gate.js`',
    '',
    goal
