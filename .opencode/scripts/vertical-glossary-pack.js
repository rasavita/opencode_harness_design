...
// .opencode/config/scaffold-packs.json (verticalPacks) is a config entry, not a new
...
// directory, not this project's own .opencode/ — hence the os.homedir() lookup
...
function loadSettings(repoRoot) {
  try {
    return JSON.parse(fs.readFileSync(path.join(repoRoot, '.opencode', 'settings.json'), 'utf8'));
  } catch (err) {
...
function loadRepoRegistry(repoRoot) {
  const registryPath = path.join(repoRoot, '.opencode', 'config', 'scaffold-packs.json');
...
function processEntry(entry, repoRoot) {
  ...
  if (!skillsDir) {
    process.stderr.write(
      ...
      `under ${os.homedir()}/.opencode/plugins — check the plugin install.\n`
    );
    return { ok: false };
  }
