...
// CLI: node .opencode/scripts/ci-ingest.js [--root DIR] [--out FILE]
...
function alignmentNotes(all) {
  ...
  notes.push(categories.has('coverage')
    ? '- CI runs coverage — verify its threshold matches the harness ratchet (80% floor, `.opencode/state/coverage-baseline*.txt`); the stricter one should win.'
