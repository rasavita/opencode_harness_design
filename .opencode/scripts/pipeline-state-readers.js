...
function readBudget(projectDir, nowMs) {
  const started = parseInt(readMarker(path.join(projectDir, '.opencode', 'state'), 'budget-start') || '', 10);
...
function readNavigation(projectDir) {
  let obj;
  try {
    obj = JSON.parse(readText(path.join(projectDir, '.opencode', 'state', 'navigation-status.json')));
  } catch (_) {
    return null;
  }
...
function readContextCache(projectDir) {
  const dir = path.join(projectDir, '.opencode', 'state', 'context-cache');
  let files;
  try {
...
function readTokenAdvisor(projectDir) {
  const records = readJsonl(path.join(projectDir, '.opencode', 'state', 'token-advisor.jsonl'));
...
function readNavTelemetry(projectDir) {
  try {
    ...
    try {
      return JSON.parse(readText(path.join(projectDir, '.opencode', 'state', 'nav-telemetry-summary.json')));
    } catch (__) {
      return null;
    }
  }
