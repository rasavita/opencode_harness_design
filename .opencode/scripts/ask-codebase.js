...
function renderAnswer(question, pack) {
  ...
  lines.push(
    ...
    '',
    '```bash',
    `node .opencode/scripts/nav-query.js pack --budget 1600 "${question.replace(/"/g, '\\"')}"`,
    `node .opencode/scripts/nav-query.js semantic "${question.replace(/"/g, '\\"')}"`,
    'node .opencode/scripts/nav-query.js hubs',
    '```',
    '',
