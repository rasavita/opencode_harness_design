...
const BASELINE = path.join(REPO, '.opencode', 'state', 'duplication-baseline.txt');
const IGNORE = [
  '**/node_modules/**', '**/dist/**', '**/.git/**', '**/coverage/**',
  '**/test/**', '**/tests/**', '**/*.test.js', '**/specs/**', '**/.opencode/state/**',
];
...
function blockMessage(d, added) {
  ...
  return [
    ...
    '.opencode/state/duplication-baseline.txt in a reviewed commit.',
    '',
  ].join('\n');
}
