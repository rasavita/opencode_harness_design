...
// CLI: node .opencode/scripts/portfolio-rollup.js <collection-dir> [--target-version X]
...
const DEFAULT_OUT = path.join('.opencode', 'attestations', 'portfolio-rollup.json');
...
// .opencode/attestations/ with no traversal / absolute / backslash / URL scheme.
function validRelPath(p) {
  return typeof p === 'string'
    && p.startsWith('.opencode/attestations/')
    && !p.includes('..')
    && !p.includes('\\')
    && !/:\/\//.test(p);
}
...
function doFetch(gh, dir, fleetEntries) {
  ...
  for (const { owner, repo } of fleetEntries) {
    const idx = fetchContents(gh, `repos/${owner}/${repo}/contents/.opencode/attestations/index.json`);
    ...
    if (!validRelPath(relPath)) {
      process.stderr.write(`portfolio-rollup: skipping ${owner}/${repo}: index path "${relPath}" is not a safe .opencode/attestations/ path\n`);
      continue;
    }
