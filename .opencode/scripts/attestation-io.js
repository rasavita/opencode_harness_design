...
function classifyGate(root) {
  const receiptFile = path.join(root, '.opencode', 'state', 'gate-receipt.json');
...
// provenance): a repo-root / .opencode/ override wins over the bundled template
...
function readStandardMap(root) {
  const candidates = [
    ...
    ['.opencode/standard-map.json', path.join(root, '.opencode', 'standard-map.json')],
    ['.opencode/templates/standard-map.json', path.join(root, '.opencode', 'templates', 'standard-map.json')],
  ];
