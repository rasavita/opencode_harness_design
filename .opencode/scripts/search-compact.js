...
// Heavy/transient trees that sit directly under a .opencode/ control dir: runtime
// state logs, run journals, and worktree checkouts. Excluded by (parent === .opencode,
// name) rather than a single top-level prefix, so nested copies inside a worktree's
// own .opencode/ are caught too. Crucially NOT a blanket '.opencode' exclusion — that hid
...
function sourceFiles(dir, prefix = '') {
  ...
  for (const entry of fs.readdirSync(path.join(dir, prefix), { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (parent === '.opencode' && CLAUDE_HEAVY_DIRS.has(entry.name)) continue;
...
if (require.main === module) {
  ...
  const pack = {
    ...
    retrieve: `node .opencode/scripts/context-retrieve.js ${stored.hash}`,
