...
// manifest, AGENTS.md, the project-tailored README.md / SCAFFOLD_README.md,
...
// `git config core.hooksPath .opencode/git-hooks` wiring (the git-hooks/ tree
// itself IS copied here), framework-pack installs, subdirectory AGENTS.md files,
// and the official-plugins enabledPlugins rebuild.
//
// CLI:
//   node scaffold-apply.js --profile <profile.json> \
//     [--plugin-source <harness .opencode dir>] [--target <project dir>]
//     [--drift-workflow]
//
// --plugin-source defaults to env HARNESS_PLUGIN_ROOT, else it is an error.
...
function writeClaudeMd(target, src, profile) {
  const body = fs.readFileSync(requireTemplate(src, 'templates/agents-md.template.md'), 'utf8');
...
// as `.opencode/templates/github-workflows/harness-drift.yml` so teams can opt in
// by copying it to `.github/workflows/`.
function copyStarterFiles(target, src) {
  const map = [
    ['templates/mcp-config.template.json', '.mcp.json'],
    ['templates/security-guidance.template.md', '.opencode/security-guidance.md'],
    ['templates/security-patterns.template.yaml', '.opencode/security-patterns.yaml'],
...
function resolveOpts(opts) {
  ...
  const pluginSource = opts.pluginSource || process.env.HARNESS_PLUGIN_ROOT;
  if (!pluginSource) fail('--plugin-source or env HARNESS_PLUGIN_ROOT is required');
  if (!fs.existsSync(path.join(pluginSource, '.opencode-plugin', 'plugin.json'))) {
    fail(`plugin source is not a harness .opencode root (no .opencode-plugin/plugin.json): ${pluginSource}`);
  }
...
function report(result) {
  ...
  process.stdout.write(`  .opencode/ tree copied (${result.scaffoldProfile} profile)\n`);
