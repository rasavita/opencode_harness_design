...
function buildDigest(root = process.cwd()) {
  ...
  const receipt = readJson(path.join(root, '.opencode', 'state', 'gate-receipt.json'));
  const nav = readJson(path.join(root, '.opencode', 'state', 'navigation-status.json'));
  const human = readJson(path.join(root, '.opencode', 'state', 'human-codebase.json'));
