...
function loadDirtyPaths(projectDir) {
  const dirty = new Set();
  const dirtyLog = path.join(projectDir, '.opencode', 'state', 'graph-dirty.jsonl');
...
function buildTaskMap(results, clusters, confidence, projectDir = null) {
  ...
  if (results.length && !tests_to_run.some((t) => t.kind === 'impact_hint')) {
    tests_to_run.push({
      kind: 'impact_hint',
      command: 'node .opencode/scripts/local-regression-gate.js',
    });
  }
  ...
  const pack = {
    ...
    fallback: {
      ...
      suggest: [
        "rg -n '<keyword>' --glob '!node_modules' --glob '!.git'",
        'refresh /code-map if source found outside index',
        'node .opencode/scripts/nav-query.js refresh  # rebuild semantic + cochange + concepts',
      ],
    },
