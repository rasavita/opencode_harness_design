...
// becomes a data file (.opencode/config/oscal-catalog.json), not a code change.
//
...
//
//   node .opencode/scripts/oscal-emit.js [--root <dir>] [--catalog <file>] [--out <file>]
...
const DEFAULT_CATALOG = path.join('.opencode', 'config', 'oscal-catalog.json');
...
function emitComponentDefinition({ manifest, catalog, harnessVersion, uuid, now }) {
  ...
  return {
    'component-definition': {
      ...
      metadata: {
        title: 'OpenCode Harness — control inventory',
...
function reportEmit(doc, outFile, catalog) {
  ...
  if (catalog) return;
  console.log('  NOTE: no OSCAL catalog bound. Add .opencode/config/oscal-catalog.json once the');
