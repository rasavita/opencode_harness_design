...
// ledger itself is gitignored (.opencode/state/*.jsonl) and a hash-chained file
...
// evidence artifacts.
//
// CLI: node .opencode/scripts/test-integrity-gate.js [--emit] [--strict]
...
function reportVacuous(strict) {
  const line =
    ...
    'Wire the PostToolUse(Bash) recorder in .opencode/settings.json so test runs are observed.\n';
  if (strict) {
