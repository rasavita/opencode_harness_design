...
//
// Usage:
//   node .opencode/scripts/scaffold-upgrade.js [--target dir] [--plugin-source dir]
//                                           [--profile core|full|brownfield]
//                                           [--apply] [--include-skills]
//
// Default target: cwd. Default plugin source: this harness's .opencode/
...
const PRESERVE_STATE_PREFIX = path.join('.opencode', 'state');
...
function planUpgrade(pluginSource, target, profileName, { includeSkills = false } = {}) {
  ...
  try {
    ...
    const staged = listRelativeFiles(stagedClaude).map((r) => path.join('.opencode', r));
    const wouldWrite = [];
    const wouldSkip = [];
    for (const rel of staged) {
      const base = path.basename(rel);
      if (PRESERVE.has(base) && rel.startsWith(`.opencode${path.sep}`) && !rel.includes(`${path.sep}templates${path.sep}`)) {
    ...
    const forcePrefixes = [
      path.join('.opencode', 'hooks'),
      path.join('.opencode', 'scripts'),
      path.join('.opencode', 'git-hooks'),
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!fs.existsSync(path.join(target, '.opencode'))) {
    process.stderr.write(
      `scaffold-upgrade: no .opencode/ under ${target}\n` +
...
function applyAndReport(pluginSource, target, plan) {
  ...
  process.stdout.write(
    'Next: review git diff under .opencode/hooks .opencode/scripts .opencode/git-hooks; ' +
