...
function main() {
  ...
  const out = opts.out || path.join(root, '.opencode', 'state', 'risk-envelope.json');
  fs.mkdirSync(path.dirname(out), { recursive: true });
  fs.writeFileSync(out, `${JSON.stringify(envelope, null, 2)}\n`);
  fs.writeFileSync(path.join(root, '.opencode', 'state', 'current-task'), `${envelope.task_id}\n`);
  fs.writeFileSync(path.join(root, '.opencode', 'state', 'current-risk-tier'), `${envelope.tier}\n`);
