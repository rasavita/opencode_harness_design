...
// CLI: node .opencode/scripts/validate-harness-manifest.js [manifest.json]
...
function emittedSensorIds(repoRoot) {
  const ids = new Set();
  const files = jsFilesUnder(path.join(repoRoot, '.opencode', 'hooks'))
    .concat(jsFilesUnder(path.join(repoRoot, '.opencode', 'scripts')));
  ...
  try {
    for (const g of require(path.join(repoRoot, '.opencode', 'hooks', 'lib', 'gate-registry')).GATE_CATALOG) {
      ids.add(g.id);
    }
