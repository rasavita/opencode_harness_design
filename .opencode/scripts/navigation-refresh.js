...
const SKIP_DIRS = new Set([
  '.git', '.opencode', 'node_modules', '.venv', 'venv', 'dist', 'build',
...
function runIndexer(projectDir) {
  const indexer = path.join(projectDir, '.opencode/skills/code-map/scripts/code_index/code_index.py');
...
function writeStatus(projectDir, status) {
  ...
  fs.writeFileSync(path.join(projectDir, '.opencode/state/navigation-status.json'), `${JSON.stringify(full, null, 2)}\n`);
  return full;
}
...
function refreshNavigation({ projectDir = process.cwd(), mode = 'scaffold' } = {}) {
  ...
  if (!indexed.ok) {
    ...
    fs.writeFileSync(path.join(projectDir, '.opencode/state/navigation-status.json'), `${JSON.stringify(status, null, 2)}\n`);
    return status;
  }
