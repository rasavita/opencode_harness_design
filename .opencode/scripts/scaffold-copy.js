...
// scaffold-copy.js — the `.opencode` tree copy + scaffold-profile selection.
//
...
//   - copies a {"type":"commonjs"} `.opencode/package.json` marker so an app whose
...
//     .opencode/git-hooks` resolves the hooks' __dirname-relative require()s.
...
// Install contents come from .opencode/config/packs.json — the same partition that
...
// added by hand, and .opencode/config/ was missed entirely (a core scaffold shipped a
...
// Copy the harness `.opencode` tree into <target>/.opencode per scaffold Step 3.
// git-hooks/ is copied in every profile (Step 8 wires it via core.hooksPath);
// package.json pins .opencode/** to CommonJS for "type":"module" apps.
function copyScaffoldTree(src, target, profileName) {
  const dotClaude = path.join(target, '.opencode');
  copyTree(path.join(src, '.opencode-plugin'), path.join(dotClaude, '.opencode-plugin'));
...
// Copy a locally-bundled framework-skill-pack's skills into <target>/.opencode/skills,
...
function copyFrameworkPackSkills(pluginSource, target, frameworkSkillPacks) {
  // pluginSource is already the harness `.opencode` root (see scaffold-apply.js's
  // resolveOpts, which verifies pluginSource/.opencode-plugin/plugin.json directly) —
  // do not join another '.opencode' segment onto it here.
  ...
  for (const key of frameworkSkillPacks) {
    ...
    copyNamedFiles(path.join(pluginSource, 'skills'), path.join(target, '.opencode', 'skills'), entry.skills);
  }
}
