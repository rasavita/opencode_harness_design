...
// Runs the /gate deterministic checks declared in .opencode/config/gate-checks.json.
//
...
//
//   node .opencode/scripts/run-gate-checks.js [--root <dir>] [--files a b ...] [--json]
...
const REGISTRY_REL = path.join('.opencode', 'config', 'gate-checks.json');
...
function runChecks(checks, projectDir, { run = null, changedFiles = [] } = {}) {
  ...
  return checks.map((c) => {
    const scriptPath = path.join(projectDir, '.opencode', 'scripts', c.script);
