...
- `--auto` (full-auto): Phase 3.5 is **skipped** — PRD straight through to PR(s), **zero** build-time human gates. The human re-appears only at merge (or the `AUTO_MERGE` key removes even that — activated locally via the `--auto-merge` flag or `AUTO_MERGE=true` env through `.opencode/scripts/auto-merge.js`, not only symphony).
...
```bash
node .opencode/scripts/pr-body.js --require-gate --title "<stories>" > /tmp/pr-body.md
...
`AUTO_MERGE=true` env through `.opencode/scripts/auto-merge.js`, or via
...
then `/auto --pod N` fans the work out so **each cluster raises its own stacked draft PR** — opened immediately, with no merge wait between waves. PR granularity is decided by `.opencode/scripts/wave-plan.js` (`pr_mode`).
...
`--auto-merge` flag or `AUTO_MERGE=true` env, resolved by `.opencode/scripts/auto-merge.js`),
