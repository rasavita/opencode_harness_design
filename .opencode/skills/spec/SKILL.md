...
```bash
node .opencode/scripts/handoff-check.js --phase spec
```
...
```bash
node .opencode/scripts/phase-digest.js --phase spec
```
...
Follow the dialogue discipline in `.opencode/skills/clarify/SKILL.md` for budget
(10 default, 15 hard cap) and `.opencode/skills/plan-review-loop/SKILL.md` for how
...
```bash
node .opencode/scripts/validate-spec-decisions.js
```
...
```bash
node .opencode/scripts/handoff-check.js --phase spec --stage render
```
...
- Rubric: read `.opencode/templates/phase-eval-rubrics.json`, key `"spec"`
...
Follow `.opencode/skills/plan-review-loop/SKILL.md`. This review is now narrower
