...
```python
import sys
sys.path.insert(0, ".opencode/skills/pe-ic-memo/scripts")
...
- **Not part of the SDLC pipeline.** This skill is not registered in `.opencode/scripts/scaffold-copy.js`'s `CORE_SKILLS`/`BROWNFIELD_SKILLS` lists, so it is not copied under the default `core` or `brownfield` scaffold profiles. (The `full` profile copies the entire skills tree wholesale, so it would be included there.) Copy `.opencode/skills/pe-ic-memo/` manually into a project on the lean profiles.
