...
```bash
node .opencode/scripts/validate-prd.js <path-to-prd.md>
```
...
Then invoke `.opencode/skills/clarify/SKILL.md`. Use it to cap the total clarification burden:
...
```bash
node .opencode/scripts/brd-adopt.js --verify
```
...
- Rubric: Read .opencode/templates/phase-eval-rubrics.json, key "brd"
...
Follow `.opencode/skills/plan-review-loop/SKILL.md` with `--phase brd`. This is a
