...
Staleness check: if `.opencode/state/graph-dirty.jsonl` exists and is non-empty, the graph is behind recent edits — the `graph-refresh` Stop hook normally drains it, but if entries remain, patch first:
`python3 .opencode/skills/code-map/scripts/code_index/code_index.py --root . --out specs/brownfield/code-graph.json --files <dirty paths>`.
...
```bash
node .opencode/skills/seam-finder/scripts/score_seams.js \
...
- Rubric: Read .opencode/templates/phase-eval-rubrics.json, key "seam"
