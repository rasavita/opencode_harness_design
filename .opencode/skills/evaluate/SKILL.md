...
Read the stack-matched verification reference under `.opencode/skills/evaluate/references/` (see the References section below) for the rigor that applies to this project's stack, plus any custom assertion helpers.
...
if [ -f "$BASELINE" ]; then
  ...
  node .opencode/scripts/perf-baseline.js --compare --endpoints "$READS" --out "$BASELINE"
  PERF_STATUS=$?
else
  # first evaluation: establish the baseline, do not fail on it
  node .opencode/scripts/perf-baseline.js --endpoints "$READS" --out "$BASELINE"
...
```bash
node .opencode/scripts/perf-baseline.js --endpoints "$READS" --out "$BASELINE"   # only on overall PASS
```
...
```bash
node .opencode/scripts/perf-baseline.js --measure \
...
`node .opencode/scripts/slo-check.js`
...
Selector and assertion rules are single-sourced in `.opencode/skills/evaluate/references/playwright-patterns.md` (the canonical Playwright reference, shared with `/test`). The rules below are a summary — defer to that file if they differ.
...
```bash
node .opencode/scripts/evidence-integrity-gate.js --group {group}
```
...
The evaluation reference pack (read by the evaluator and design-critic agents) lives under `.opencode/skills/evaluate/references/`:
