...
- **Never touch `AGENTS.md`, `.mcp.json`, or a `.opencode/settings*.json` file.**
...
Run:
```bash
node .opencode/scripts/promote-recommendation.js --check <recommendation-id>
```
...
If `target` names `AGENTS.md`, `.mcp.json`, or a `.opencode/settings*.json`
