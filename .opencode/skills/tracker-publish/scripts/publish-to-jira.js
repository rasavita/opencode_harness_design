...
function validateConfig(config) {
  if (!config.baseUrl || !config.projectKey || /^replace-with-/.test(String(config.projectKey))) {
    throw new Error('tracker.base_url and tracker.project_key must be set in .opencode/tracker-config.json.');
  }
...
async function main() {
  ...
  const trackerMap = readJson(path.join(projectRoot, '.opencode/state/tracker-map.json'));
  const trackerConfig = readJson(path.join(projectRoot, '.opencode/tracker-config.json'));
  ...
  const readBody = (group, groupId) => fs.readFileSync(
    path.join(projectRoot, group.body_file || `.opencode/state/tracker-runs/group-${groupId}.md`), 'utf8');
  ...
  if (!args.dryRun) {
    fs.writeFileSync(path.join(projectRoot, '.opencode/state/tracker-map.json'), JSON.stringify(trackerMap, null, 2) + '\n');
  }
