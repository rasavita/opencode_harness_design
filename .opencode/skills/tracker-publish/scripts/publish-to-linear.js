...
const trackerMapPath = path.join(projectRoot, '.opencode/state/tracker-map.json');
const trackerConfigPath = path.join(projectRoot, '.opencode/tracker-config.json');
...
async function main() {
  ...
  for (const [groupId, group] of Object.entries(trackerMap.groups || {})) {
    ...
    const bodyFile = path.join(projectRoot, group.body_file || `.opencode/state/tracker-runs/group-${groupId}.md`);
...
Reads .opencode/state/tracker-map.json and creates Linear issues for every group
