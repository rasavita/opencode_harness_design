...
If a tracker is configured (`.opencode/tracker-config.json`), run
...
At the start of Phase 0, write `.opencode/state/current-sprint` (the resolved
sprint number, e.g. `2`) and update `.opencode/state/sprint-phase` at the start
...
printf '%s' "N" > .opencode/state/current-sprint
printf '%s' "<phase-name>" > .opencode/state/sprint-phase
```
