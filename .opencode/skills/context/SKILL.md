...
```bash
node .opencode/scripts/context-pack.js "$ARGUMENTS"
```

Optional flags:

```bash
node .opencode/scripts/context-pack.js --budget 1600 "$ARGUMENTS"
node .opencode/scripts/context-pack.js --diff --budget 1600 "$ARGUMENTS"
node .opencode/scripts/context-pack.js --depth 2 --diff --budget 1600 "$ARGUMENTS"
node .opencode/scripts/context-pack.js --no-receipt --budget 1600 "$ARGUMENTS"
```

Unified facade (pack + graph + impact + semantic + co-change + refresh):

```bash
node .opencode/scripts/nav-query.js pack --budget 1600 "$ARGUMENTS"
node .opencode/scripts/nav-query.js symbol validateSession
...
node .opencode/scripts/nav-query.js cochange src/auth/session.py
node .opencode/scripts/nav-query.js semantic "session validation"
node .opencode/scripts/nav-query.js refresh   # TF-IDF index + co-change + concept pages
```

Optional MCP: merge `.opencode/templates/mcp-nav.snippet.json` into `.mcp.json`
...
| `--no-receipt` | Do not write `.opencode/state/context-pack-last.json` |
...
A successful run also writes `.opencode/state/context-pack-last.json` (session
