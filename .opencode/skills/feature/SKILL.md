...
Lane resolution is deterministic — `node .opencode/scripts/feature-lane.js "<args>"`
...
Before Step 2 (Decompose), read `.opencode/state/learned-rules.md`. If it exists
...
`node .opencode/scripts/feature-lane.js "<args>"`.
...
1. **Deterministic seam-confidence gate.** After the DeepWiki is fresh, run
   ...
   `node .opencode/scripts/seam-confidence.js --graph specs/brownfield/code-graph.json --goal "<request>"`.
...
```bash
node .opencode/scripts/risk-envelope.js --task <story-or-epic-id> \
  --story <story-file> --graph specs/brownfield/code-graph.json \
  --file <expected-path> [--file <expected-path> ...]
```

Read `.opencode/state/risk-envelope.json` and carry its `tier`,
...
```bash
node .opencode/scripts/task-envelope.js create --task <story-or-epic-id> \
...
node .opencode/scripts/task-lifecycle.js transition active "approved feature execution begins"
```
...
```bash
node .opencode/scripts/impact-classifier.js --story <story-file> --graph specs/brownfield/code-graph.json
```
...
- **Single-story lane:** build a one-issue map with
  `node .opencode/skills/tracker-publish/scripts/single-story-map.js`'s
  `buildSingleStoryMap(...)` (or `tracker-publish --granularity single`), write
  the AC to `.opencode/state/tracker-runs/group-<storyId>.md` and the map to
  `.opencode/state/tracker-map.json`, then publish with the unchanged
...
```bash
node .opencode/scripts/context-pack.js --diff --budget 1600 "<feature request>"
```
