...
```bash
node .opencode/scripts/diagnostics-shard.js --tool tsc --from-file /tmp/harness-tsc.txt
# or auto-detect:
node .opencode/scripts/diagnostics-shard.js --auto --from-file /tmp/harness-tsc.txt
```

Writes:

- `.opencode/state/diagnostics/errors.jsonl` — one JSON object per error  
- `.opencode/state/diagnostics/shards.json` — shards grouped by package/module  
...
1. Create `.opencode/state/parallel-implement.lock` (empty).  
...
5. Commit owned paths only when the shard's diagnostic count for those files drops (optional mid-queue commits; at least one commit before the final suite). After dual review, prefer:
   ```bash
   git commit -m "$(node .opencode/scripts/review-commit-msg.js --subject "fix $(shard.id) diagnostics" --from-audit specs/reviews/adversarial-review-audit.json)"
   ```
...
If the queue failed because agents stubbed, ran full-suite mid-shard, or used destructive git, append a rule to `.opencode/state/process-rules.md` (workflow constraint, not code style).
...
| Path | Purpose |
|------|---------|
| `.opencode/state/diagnostics/errors.jsonl` | Flat error list |
| `.opencode/state/diagnostics/shards.json` | Work queue + progress input |
