---
name: retro
description: Review this harness's own recent loop-health signals and recommend scored, evidence-backed improvements to the harness itself (skills, gates, sensors, AGENTS.md rules) — never to product code. Use after a `/auto`/`/gate` run, on a drift cadence, or on demand when the loop feels off. Interactive-only — recommends and records human decisions; does not apply anything.
...
or `AGENTS.md` itself — implementing an approved recommendation is a normal
...
- `.opencode/state/learned-rules.md` and `.opencode/state/process-rules.md` —
  monotonic rule stores; a rule that keeps recurring across scorecards is
  itself evidence for a recommendation (e.g. "promote this to AGENTS.md").
...
- `npm run sensor-comprehension-debt` — the merged-but-unread STOCK meter:
  reads `.opencode/state/merge-provenance.jsonl`, keeps the `human_reviewed:false`
...
Run `npm run loop-health` (or `node .opencode/scripts/loop-health.js --root
...
```json
{
  ...
  "evidence": ["specs/retro/loop-health.md#signals", ".opencode/hooks/lib/loop-health.js:83"],
  "status": "proposed"
}
```
...
- This skill never touches product code in a scaffolded target project — its
  `target` values name harness surfaces (skills, gates, sensors, `AGENTS.md`)
