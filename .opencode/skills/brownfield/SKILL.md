...
Run the `/code-map` skill (`.opencode/skills/code-map/SKILL.md`) — its Steps 1–3 are the **single source of truth** for producer detection (AST indexer → Understand-Anything import → regex fallback), the exact commands, and the rendering of `symbol-map.md`, `dependency-graph.md`, and `coupling-report.md`. Do not restate or improvise those commands here; if anything in this skill disagrees with code-map's SKILL.md, code-map wins.
...
node .opencode/scripts/nav-brownfield-maps.js --root . --goal "<goal-or-empty>"
...
```bash
node .opencode/scripts/nav-query.js pack --diff --budget 1600 "<goal>"
```
...
```bash
node .opencode/scripts/ci-ingest.js --root .     # → specs/brownfield/ci-map.md
node .opencode/scripts/flag-scan.js --root .     # → specs/brownfield/flag-inventory.md
```
...
```bash
node .opencode/scripts/perf-baseline.js          # → specs/brownfield/perf-baseline.json
# after a change: node .opencode/scripts/perf-baseline.js --compare   (exit 1 on p95 regression)
```
...
```bash
node .opencode/scripts/modularity-pack.js     # → specs/brownfield/modularity-pack.md (+ .json)
```
...
```bash
node .opencode/scripts/record-modularity-review.js
```

This writes/updates `.opencode/state/modularity-review-marker.json` with `{timestamp, unstableHubIds}` — the current unstable-hub set at the moment this review ran. Skip only if it errors for lack of a code-graph (already handled loudly by the script itself).
...
- **Within a group:** multi-story groups (≥ 2 stories) fan out into parallel teammates — see `.opencode/agents/generator.md` Rule 2.
- **Across groups:** independent dependency groups run concurrently as group-orchestrators (up to 3 by default) — see Section 4B of `.opencode/skills/auto/SKILL.md`.
...
```bash
node .opencode/scripts/naming-clusters.js
```
...
- Rubric: Read .opencode/templates/phase-eval-rubrics.json, key "brownfield"
