...
```bash
node .opencode/scripts/validate-design-decisions.js
```
...
```bash
node .opencode/scripts/handoff-check.js --phase design --stage render
```
...
```bash
node .opencode/scripts/validate-canvas.js specs/design/reasons-canvas.md
```
...
```bash
node .opencode/scripts/vocabulary-check.js \
  --glossary CONTEXT.md \
...
- Rubric: Read .opencode/templates/phase-eval-rubrics.json, key "design"
