...
2. If it exists, refresh the pack: `node .opencode/scripts/modularity-pack.js`.
...
6. If the agent completed (not `inconclusive`, and this step wasn't skipped
   ...
   since this review): `node .opencode/scripts/record-modularity-review.js
...
```bash
node .opencode/scripts/contract-drift-gate.js --spec specs/design/api-contracts.schema.json
```
...
- Rubric: Read .opencode/templates/phase-eval-rubrics.json, key "design-delta"
...
`.opencode/skills/plan-review-loop/SKILL.md` with `--phase design` — the brief
...
```bash
node .opencode/scripts/plan-approval.js record --phase design --verdict approved \
