...
> A **disposable artifact** lane (see AGENTS.md → *Disposable Artifacts*). It does **not** spawn the planner, generator, or evaluator; it produces **no** machine-readable schemas, mockups, trace spines, or grounding gates; it runs **no** ratchet loop and **no** security review. There is no story prerequisite. Skip every numbered step below — they belong to full mode only.
