...
**Human approval is required before proceeding to `/auto`.** Run it as a dialogue, not a single question — follow `.opencode/skills/plan-review-loop/SKILL.md` with `--phase design`.
...
```bash
node .opencode/scripts/plan-approval.js check --phase design
```
