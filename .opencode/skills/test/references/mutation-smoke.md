...
It is the automated, repeatable form of the manual checkpoint in `pinning-down-behavior` ("flip a behavior, watch the pin fail"). Runner: `.opencode/scripts/mutation-smoke.js`.
...
node .opencode/scripts/mutation-smoke.js \
...
node .opencode/scripts/mutation-smoke.js --files src/auth.ts --dry-run
```
