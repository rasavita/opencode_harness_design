...
`test-strategy.md` gives the layer model (unit / integration / E2E) and a short boundary checklist. This file is the technique that fills that checklist out. Read both. The deterministic companion is `.opencode/scripts/constraints-extract.js`, which mines schema constraints into machine-checkable obligations (see "Constraint obligations" below).
