...
sha256-integrity-hashed attestation bundle committed under `.opencode/attestations/`.
...
- At `/gate` or in CI, after the verify/gate outputs for a commit exist, to write the committed
  evidence record — then commit `.opencode/attestations/<sha>.json` and `index.json`.
...
```
node .opencode/scripts/generate-attestation.js            # write .opencode/attestations/<sha>.json (+ index.json)
node .opencode/scripts/generate-attestation.js --force    # overwrite an existing record for this SHA
node .opencode/scripts/generate-attestation.js --verify .opencode/attestations/<sha>.json
node .opencode/scripts/generate-attestation.js --json     # print the bundle instead of a summary line
```
...
- **Inputs** — three sources (branch-protection verify, deploy-gate verify, gate receipt), each classified
  ...
  inventory + per-control clause from `harness-manifest.json`, the `.opencode/state/*-baseline.*` ratchets,
...
`.opencode/attestations/<sha>.json` (immutable, one per attested commit) and `.opencode/attestations/index.json`
(`{ entries, integrity }`, deduped + integrity-checksummed) are committed under the tracked `.opencode/` tree —
...
Each control resolves a `standard_ref` from `.opencode/templates/standard-map.json` — a neutral clause
...
root or `.opencode/`; it overrides the template default and its `id` is stamped into the bundle's
