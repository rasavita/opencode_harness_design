...
1. **Plan (read-only, run first):**
   ```bash
   node .opencode/scripts/provision-protection.js plan
   ```
...
3. **Apply** (org-admin; mutates GitHub — idempotent create-or-update):
   ```bash
   node .opencode/scripts/provision-protection.js --apply                     # org scope
   node .opencode/scripts/provision-protection.js --apply --repo owner/repo    # single repo
   node .opencode/scripts/provision-protection.js --apply --fleet fleet.json   # repo fleet
   ```
...
1. **Plan (read-only, run first):**
   ```bash
   node .opencode/scripts/provision-environments.js plan --repo owner/repo
   ```
...
3. **Apply** (org-admin; idempotent PUT):
   ```bash
   node .opencode/scripts/provision-environments.js --apply --repo owner/repo   # single repo
   node .opencode/scripts/provision-environments.js --apply --fleet fleet.json  # repo fleet
   ```
4. **Verify** (writes `specs/reviews/deploy-gate-verify.json`, non-zero on drift):
   ```bash
   node .opencode/scripts/provision-environments.js --verify --repo owner/repo
   ```
...
node .opencode/scripts/fleet-retrofit.js --fleet fleet.json --apply    # apply then re-verify
node .opencode/scripts/fleet-retrofit.js --fleet fleet.json --json     # also print the report object
```
