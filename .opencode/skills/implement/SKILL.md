...
If story metadata, component ownership, or API/data contracts conflict, invoke `.opencode/skills/clarify/SKILL.md` before planning implementation. Keep clarification bounded:
...
Read `.opencode/skills/code-gen/SKILL.md` in full. Its core quality principles apply to every line of code produced. Inject the full text into every teammate prompt.
...
The agent-team execution protocol is defined **once**, in `/auto` SECTION 4 (Agent Team Execution) — `.opencode/skills/auto/SKILL.md`. Follow it verbatim in **standalone mode** (skip SECTION 3's sprint-contract negotiation): the mandatory team spawn for 2+ story groups (1 teammate per story, max 5 concurrent, batch the remainder), the orchestrator and teammate spawn prompt templates, model tiering, the shared-type dependency handshake, and the spawn-evidence logging to `.opencode/state/iteration-log.md`. Do not restate or improvise that protocol here — if this skill and SECTION 4 ever disagree, SECTION 4 wins.
...
```bash
node .opencode/scripts/review-tier.js --files <changed_production_file_count> --lines <changed_line_count> [--security-boundary]
```
...
```bash
git commit -m "$(node .opencode/scripts/review-commit-msg.js \
...
- **Ignoring learned rules:** Failing to inject `.opencode/state/learned-rules.md` recreates decisions the team has already made, causing style and pattern drift.
