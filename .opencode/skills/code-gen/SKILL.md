...
When generated code calls any external API (third-party services, partner APIs, cloud services), follow these rules. See `.opencode/skills/code-gen/references/api-integration-patterns.md` for full templates.
...
- **Worked patterns.** For the concrete shape of the principles above, read `.opencode/skills/code-gen/references/code-patterns.md`.
- **LLM calls.** When the code you're writing calls an LLM, read `.opencode/skills/code-gen/references/llm-integration.md` first — structured output is mandatory and the failure modes are specific.
- **Production code.** For configuration, error-envelope, and layering rules, read `.opencode/skills/code-gen/references/production-standards.md`.
- **Before handing off.** The mistakes that fail review are listed in `.opencode/skills/code-gen/references/review-failure-gotchas.md` — check it when a change is ready, not while drafting.
