...
The numeric gate is single-sourced: the **floor is 80%, target 100%**, enforced by `/auto` Gate 3 and the coverage hook (see `.opencode/skills/code-gen/SKILL.md`). This file defers to those for the exact number. A failing coverage gate blocks merge — it is not advisory.
...
enumeration, and idempotency — see `.opencode/skills/test/references/test-design.md`.
...
The red-green-refactor / vertical-slice discipline (one failing behavior test → minimum code → pass → repeat) is canonical in `.opencode/skills/code-gen/SKILL.md` and the `superpowers:test-driven-development` skill — follow those rather than a second copy here. This file's contribution is the layer model and the boundary-condition checklist above, not the TDD loop.
