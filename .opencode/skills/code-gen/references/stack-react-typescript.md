...
- **Separation** (matches the generated `frontend/AGENTS.md`): API calls live in `src/api/`, never `fetch` directly inside a component; shared types in `src/types/`. Components render state; the api layer owns I/O.
