...
2. **Update per-group state:** Write proposed `features.json` updates to `.opencode/state/wave-{N}/group-{group}/features-update.json` and the per-group `iteration-log.md` and `learned-rule-candidates.md`. Do NOT touch parent-owned files.
