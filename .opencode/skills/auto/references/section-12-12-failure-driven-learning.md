...
Extract a new rule when the same error type (by category from SECTION 6) appears **2 or more times** in `.opencode/state/failures.md`. Check after every failure entry.

### Rule Format

Append to `.opencode/state/learned-rules.md`:
