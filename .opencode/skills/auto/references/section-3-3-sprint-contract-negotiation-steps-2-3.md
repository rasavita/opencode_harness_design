...
- **Validate before it freezes.** After the evaluator writes the final contract, run `node .opencode/scripts/validate-contract.js sprint-contracts/{group}.json`, then `node .opencode/scripts/verification-matrix-gate.js --phase contract --group "$GROUP_ID"`. A non-zero exit means the contract is structurally malformed or missing required matrix coverage — re-run Step 3 once with the validator output attached. Do not proceed to execution with an invalid contract: on every commit that stages source files during an active sprint group, the pre-commit hook deterministically re-validates the contract's schema shape and — when `specs/test_artefacts/verification-matrix.json` exists — re-runs the verification-matrix `executed` phase, so a malformed contract or missing/stale runtime evidence blocks the commit regardless of whether this step was run.
...
`node .opencode/scripts/contract-accessibility-default.js sprint-contracts/{group}.json`
...
After running the normalizer, re-run `node .opencode/scripts/validate-contract.js sprint-contracts/{group}.json` to confirm the (possibly-augmented) contract is still schema-valid before moving to execution.
