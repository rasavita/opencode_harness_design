...
1. Read `.opencode/state/iteration-log.md` — there must be a `team_mode` decision for multi-story groups.
...
- Learned rules (from `.opencode/state/learned-rules.md` — inject verbatim)
- Process rules (from `.opencode/state/process-rules.md` when non-empty — inject verbatim; workflow constraints, not code style)
- Quality principles (from `.opencode/skills/code-gen/SKILL.md`)
...
- If story involves external API: `.opencode/skills/code-gen/references/api-integration-patterns.md`
...
The orchestrator runs on the **session model** (whatever `/model` is set to — Opus 5). Subagent models are pinned per agent in `.opencode/agents/<name>.md` frontmatter (`model:`), stamped from the cost-posture preset in `project-manifest.json` → `execution.model_tier` (default `balanced`):
...
Re-stamp after editing the manifest: `node .opencode/scripts/model-tier.js <preset> --apply .opencode/agents`. Full rationale + decision rule: `docs/model-allocation.md`.
