...
For each entry, check whether the prefix matches any directory under `.opencode/skills/`:

```bash
COUNT=$(find .opencode/skills -maxdepth 1 -type d -name '<prefix>*' | wc -l | tr -d ' ')
```
...
Write a small status file at `.opencode/state/framework-packs-install.json` recording the last-run timestamp and per-pack state so subsequent runs and Step 10 scaffold reports can read it:
