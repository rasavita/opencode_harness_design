...
4. **Prove reversibility.** Run the migration **and its down-migration** against a seeded copy of the schema before marking the work done — the harness ships the runner:
   ...
   ```bash
   .opencode/scripts/migration-roundtrip.sh --ephemeral-postgres   # docker spins a disposable DB
   # or: DATABASE_URL=<disposable-db-url> .opencode/scripts/migration-roundtrip.sh
   ```
