...
Aggregates a collection of per-repo Increment-4a attestations (`.opencode/attestations/<sha>.json`)
...
```
node .opencode/scripts/portfolio-rollup.js <collection-dir>                      # roll up a local collection
node .opencode/scripts/portfolio-rollup.js <collection-dir> --target-version 2.5.0
node .opencode/scripts/portfolio-rollup.js <collection-dir> --fetch --fleet fleet.json
node .opencode/scripts/portfolio-rollup.js --verify .opencode/attestations/portfolio-rollup.json
```
...
The rollup is written to `--out` (default `.opencode/attestations/portfolio-rollup.json`). Like the 4a
