...
- You do **not** want to overwrite `project-manifest.json`, `program.md`, settings, or `.opencode/state/`.
...
1. Resolve the harness plugin source (the `.opencode` tree of the harness version you want):
   - Packaged: `dist/skus/harness-core` (or full)
   - Contributor clone: `~/opencode_harness_design/.opencode`
...
4. In the target project, review `git diff` under `.opencode/hooks`, `.opencode/scripts`, `.opencode/git-hooks`, `.opencode/agents`.
...
|---|---|
| `project-manifest.json` (project root) | `.opencode/hooks/**` |
| `.opencode/state/**` | `.opencode/scripts/**` |
| `program.md`, settings (skipped) | `.opencode/git-hooks/**`, agents |
