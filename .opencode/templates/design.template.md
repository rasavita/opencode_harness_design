...
# OpenCode Harness Engine v5 — Design Reference
...
|------------------|-------------------------------|----------------------------------------|
| Planner          | `.opencode/agents/planner.md`   | Sprint planning, story breakdown, architecture |
| Generator        | `.opencode/agents/generator.md` | Feature implementation; also test authoring (`skills/test/references/test-authoring.md`) and UI mockups (`skills/design/references/ui-mockups.md`) |
| Evaluator        | `.opencode/agents/evaluator.md` | Runtime mode: API + Playwright verification. Artifact mode: rubric-scores planning docs |
| Design Critic    | `.opencode/agents/design-critic.md` | Visual design scoring (Karpathy loop) |
| Security Reviewer| `.opencode/agents/security-reviewer.md` | Vulnerability auditing             |
| Codebase Explorer| `.opencode/agents/codebase-explorer.md` | Read-only brownfield discovery     |
...
One consolidated hook per event (each dispatches its checks in-process from `.opencode/hooks/lib/`):
...
| `prepare-commit-msg` | Harness-Lane/Mode/Iteration/Group trailers from `.opencode/state/current-*` markers |
