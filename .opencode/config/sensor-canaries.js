...
function withTempPatterns(rules, fn) {
  ...
  try {
    ...
    fs.writeFileSync(path.join(dir, '.opencode', 'security-patterns.json'), JSON.stringify(rules));
    return fn(dir);
  } finally {
...
const CANARIES = [
  ...
  },
  {
    ...
    why: 'a AGENTS.md write must be caught; a README.md write must not',
    run() {
      ...
      try {
        return {
          bit: P.prefixCacheViolation(root, path.join(root, 'AGENTS.md')) !== null,
  ...
  },
  {
    ...
    run() {
      ...
      const root = path.join(__dirname, '..', '..');
      return {
        bit: T.machineryViolation(root, path.join(root, '.opencode/hooks/pre-write-gate.js')) !== null,
