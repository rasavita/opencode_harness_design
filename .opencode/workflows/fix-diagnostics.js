 ...
 *   .opencode/skills/fix-from-diagnostics/SKILL.md
 *
...
await phase('Capture', async () => {
  log(
    `Build diagnostics queue for tool=${toolHint}. ` +
      ...
      `node .opencode/scripts/diagnostics-shard.js --auto --from-file <capture> ` +
      `(or --tool tsc|eslint|ruff|mypy). ` +
      `Confirm .opencode/state/diagnostics/shards.json has total_errors and shards.`
  )
  await agent(
    `Capture the project's failing type/lint command output for: ${toolHint}. ` +
      `Write the capture to .opencode/state/diagnostics/last-capture.txt if needed. ` +
      `Run node .opencode/scripts/diagnostics-shard.js --auto --from-file <that file> ` +
...
await phase('Shard fix', async () => {
  ...
  await agent(
    `Read .opencode/state/diagnostics/shards.json. ` +
      ...
      `node .opencode/scripts/merge-review-verdicts.js --policy union. ` +
      `4) Optional commit using: node .opencode/scripts/review-commit-msg.js --subject "fix <shard.id> diagnostics" --from-audit specs/reviews/adversarial-review-audit.json ` +
      `when dual review ran. ` +
      `Do NOT run the full monorepo suite between shards. ` +
      `Follow .opencode/skills/fix-from-diagnostics/SKILL.md and .opencode/state/process-rules.md.`,
...
await phase('Process learn', async () => {
  await agent(
    `If this run saw stub-to-green, destructive git, or full-suite thrash mid-shard, ` +
      `append a short process rule to .opencode/state/process-rules.md (workflow constraint, not code style). ` +
      `If the workflow itself should change (new deny, different shard size), document the edit ` +
      `for a human to apply to .opencode/workflows/fix-diagnostics.js — fix the process, not only the tree. ` +
