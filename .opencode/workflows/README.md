...
2. Append a **process rule** to `.opencode/state/process-rules.md`.  
3. Patch `.opencode/workflows/fix-diagnostics.js` (or the skill) so the next run cannot repeat the failure.
...
- **Kill-switch:** an org admin (or `~/.opencode/settings.json` /
  ...
  do not add it to `.opencode/settings.json`.
