...
Phase order is owned by `/build` (`.opencode/skills/build/SKILL.md`): Brownfield discovery (existing codebases) → BRD → Spec → Design + Test-planning (parallel) → state init → deploy artifacts (docker mode) → autonomous execution via `/auto` (ratchet loop) → E2E test generation → README. Do not sequence phases from this file.
