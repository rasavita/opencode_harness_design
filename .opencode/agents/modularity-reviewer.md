...
- `specs/brownfield/modularity-pack.md` (+ `.json`) — the deterministic pack: hubs (each pre-tagged `likely-legitimate` or not), import cycles, and duplication candidates. If it is missing, run `node .opencode/scripts/modularity-pack.js` first (it needs `specs/brownfield/code-graph.json`).
