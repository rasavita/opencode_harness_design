...
Pre-commit prints `pre-commit: sensor_tier=<tier>` at start. New or refactored **BLOCKED** messages should use `.opencode/hooks/lib/gate-result.js` `formatBlock()`:
...
Waivers live at `specs/reviews/sensor-waivers.json` and must match `.opencode/templates/sensor-waivers.schema.json`. A waiver is not a suppression forever; it is a reviewed note with an expiry condition.
